...
FMT_VCS_STATUS="%{$oxide_turquoise%}on %{$oxide_red%} %b%u%c%{$oxide_reset_color%}"
...
# testing
USERAT="%{$fg_bold[white]%}%n%{$reset_color%} at %{$fg_bold[yellow]%}%m%{$reset_color%} in"
# Oxide prompt style.
PROMPT=$'\n%{$USERAT $fg_bold[green]%}%~%{$oxide_reset_color%} ${vcs_info_msg_0_}%{$reset_color%}\n%(?.%{%F{white}%}.%{$oxide_red%})%(!.#.→ )%{$oxide_reset_color%} '

# # Original
# PROMPT=$'%\n%{$USERAT $oxide_limegreen%}%~%{$oxide_reset_color%} ${vcs_info_msg_0_}\n%(?.%{%F{white}%}.%{$oxide_red%})%(!.#.❯)%{$oxide_reset_color%} '
